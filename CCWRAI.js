...
Game.registerMod('CCWRAI',{
	...
	endRun:async function(){
		...
		console.log(`RUN ${rNum} COMPLETE: ${Game.handmadeCookies} Cookies - Total Reward: ${Math.round(totalReward*100)/100} --> ${dps.substring(2)} --> ${iteration} Steps (${nInvalid} Invalid) with eps: ${Math.round(eps*1000)/1000} in ${this.beautifyTime(Date.now() - segTime)}`)
		segTime = Date.now();
		await this.train();
