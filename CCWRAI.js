...
const pi2index = [1, 2, 6, 7, 8, 17]; //same thing but for permute options (use: index2id[pi2index[i]] ) --> click, cursor, cursor_up1 (req 1), cursor_up2 (req 1), cursor_up3 (req 10)
...
const verbose = true; //flag for including more console outputs for debugging
...
Game.registerMod('CCWRAI',{
	...
	continueDetermining:function(){
		...
		if (stop) {
			...
			for (let i = starti; i < pi2index.length - 1; i++) {
				let p = pi2index[i];
	...
	loadModel:async function(v, r) {
		this.network = await tf.loadLayersModel(`downloads://trained-model-v${v}-run${r}`);
		if (this.network) {
			console.log(`Successfully loaded trained-model-v${v}-run${r}!`);
		} else {
			console.warn(`ERROR: Failed to load trained-model-v${v}-run${r}!`);
		}
	},
    ...
    runAndReward:function(action) {
    	...
    	//r -= 100;
    	return r / 10000; //reduce value of r to avoid exploding gradient
    },
	...
	endRun:async function(){
		...
		console.log(`RUN ${rNum} COMPLETE: ${Game.handmadeCookies} Cookies - Total Reward: ${Math.round(totalReward*100)/100} --> ${dps.substring(2)} --> ${iteration} Steps (${nInvalid} Invalid) with eps: ${eps} in ${this.beautifyTime(Date.now() - segTime)}`)
		segTime = Date.now();
		await this.train();
